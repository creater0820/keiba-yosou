...
with st.expander("🆕 v1.13.0 Phase 2: 期待値(EV)・公正オッズについて", expanded=False):
    st.markdown(
        r"""
Phase 1 の確率を使い、**オッズと組み合わせて「買って得かどうか」** を見える化しました。
ここでも rating ロジック・確率・◎○▲△・買い目は **一切変えていません**(純粋追加)。

- **公正オッズ(fair odds)**: その馬の予測確率なら「最低これだけのオッズが付けば
  損得ゼロ」という分岐点。オッズ入力なしで自動計算されます。

$$ \text{公正単勝オッズ} = \frac{1}{\text{単勝確率}} $$

- **EV(期待値)**: JRA の実オッズを入力すると計算される「1 円あたりの期待損益率」。

$$ \text{EV}_{\text{単勝}} = \text{単勝確率} \times \text{オッズ} - 1 $$

  EV > 0 なら理論上プラス。お買い得フラグ: 🎯 高期待値(EV≥20%)/ ⭐ お買い得(≥10%)
  / 💡 プラス期待値(>0、オッズ 2.0 倍以上)。閾値はサイドバー「💰 EV 表示設定」で調整可能。

- **DC 形式の制約**: 当日出馬表 CSV には単勝オッズが含まれない(v1.7.4 確定)ため、
  公正オッズは常時自動表示、EV は予想結果下部「💰 期待値(EV)計算」でオッズを
  手入力すると計算します。

**注意**: EV は「予測確率に基づく期待値」であり、未来の的中を保証しません。
オッズは締切間際に動くため入力タイミングに注意してください。

実装: `utils/ev_engine.py`(`compute_race_evs` / `kelly_fraction`)。
rating / probability エンジンとは完全に分離しています。

**今後の予定**: Phase 3 = ◎○▲△を確率・EV ベースで再定義、Phase 4 = Kelly 基準の
推奨投資額(関数は Phase 2 で用意済み)。
        """
    )
