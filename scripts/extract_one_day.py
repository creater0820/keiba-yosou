...
クリアする列(レース結果由来):
    [19] 用途不明だが結果系の値、[20]/[21] 着順(及び複製)、[22-23] 着差・着差秒、
    [25-26] 走破タイム、[27-31] 時計指数・通過順、[32] 上がり3F
    ※ [24] = 馬番 は朝時点で確定しているのでクリアしない。
"""
...
# 真の着順は [20]/[21] で、[19] は用途不明だが結果系。3列ともクリアする。
# [24] は馬番なので朝時点で確定済み → クリアしない。
RESULT_COLS_TO_CLEAR = [19, 20, 21, 22, 23, 25, 26, 27, 28, 29, 30, 31, 32]
