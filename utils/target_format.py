...
#   [17]    斤量
#   [18]    出走頭数(全行同値)
#   [19]    用途不明の値(実 JRA 結果と合わない。レース結果由来っぽいので
#            CSV 加工時はクリア対象として安全側に倒す)
#   [20]    着順(2桁ゼロ埋め、例 '01' = 1着)            ← finishing_position
#   [21]    着順(同 [20] の複製、JV-Link が冗長に出力)
#   [22-23] 着差・着差秒
#   [24]    馬番(1〜N、レース内ユニーク)              ← horse_number
...
RACES_COL: dict[str, int] = {
    ...
    # かつて [19] を着順としていたが、それは別の値(用途不明)で実着順とは違う。
    "finishing_position": 20,
    # 馬番は [24]。per-race で 1..N の置換になり、各馬で安定する。
    "horse_number":       24,
    "time_seconds":       25,
    "last_3f":            32,
    "weight":             33,
