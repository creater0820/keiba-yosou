...
def is_dc_format(text: str) -> bool:
    ...
    DC 形式の特徴:
    - ヘッダー行なし
    - col[1..9] は数値(コード値・距離・指数など)
    - 列数 30〜80 前後(典型 46 列 = 10 ベース + 5 × 7 過去走)
    - 1 列目が 10 桁の ID(2桁場 + 2桁年 + 2桁開催 + 2桁R + 2桁馬番)

    判定ルール(全て AND):
    - 列数が JV-Link 52 列形式ではない(明確に区別)
    - 列数が 30 以上 80 未満(極端な誤認回避)
    - 1 列目が 10 文字で、先頭 2 桁が既知 JRA 場コード(01〜10)であること
    - 1 列目の 年(2-4桁目)/ R番(6-8桁目)/ 馬番(8-10桁目)が数字であること
    - 1 行目の **2〜10 列目(col[1..9])** がすべて数値として読めること

    ⚠ v1.13.1: 旧実装は col[0] 全 10 桁を `isdigit()` で要求していたが、
    TARGET の DC 形式は **開催回コード(4-6桁目)が英数字になりうる**
    (例: `05262B0101` の "2B")。複数開催日(回次が進んだ開催)で顕在化し、
    DC として検出されず header_csv 経路に落ちて読込失敗していた。
    開催コードは parse 側で opaque な文字列(horse_id 構築のみ)として扱うため、
    検出では col[0] 全体の数字要件を外し、場コード + R番 + 馬番 + col[1..9] の
    数値性で判定する(複数会場・複数開催回でも安定)。
    ...
    if not (30 <= n_fields < 80):
        return False
    # 1 列目: 10 文字の ID(開催回コード 4-6桁目は英数字を許容)
    id0 = fields[0]
    if len(id0) != 10:
        return False
    # 先頭 2 桁が既知 JRA 場コード(01〜10)
    if id0[:2] not in JRA_COURSE_BY_CODE:
        return False
    # 年(2-4桁)/ R番(6-8桁)/ 馬番(8-10桁)は数字必須(開催コード 4-6桁は除外)
    if not (id0[2:4].isdigit() and id0[6:8].isdigit() and id0[8:10].isdigit()):
        return False
    # col[1..9] がすべて数値として読めること(col[0] は ID なので除外)
    for v in fields[1:10]:
        if v == "":
            continue
        try:
            int(v)
