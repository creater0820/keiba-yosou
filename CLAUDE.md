...
- **v1.13.0 Phase 2**(現行、2026-05): **期待値(EV)計算 + 公正オッズ + お買い得
  馬発見**。v1.12.0 の確率を使い、オッズと組み合わせて「買って得か」を可視化する。
  rating / 確率ロジックは **一切不変**、純粋追加。

  * **方針**: (1) 既存 rating / 確率 完全維持、(2) 公正オッズ(オッズ不要)+
    マーケットオッズ手入力 EV のハイブリッド、(3) ◎○▲△マーク・買い目は不変
    (Phase 3 で再検討)、(4) Kelly は関数だけ用意(Phase 4 で UI 表示)。

  * **新規モジュール `utils/ev_engine.py`(完全分離)**:
    - 公正単勝オッズ = 1 / win_prob、公正複勝オッズ ≈ 1 / place_prob
    - 単勝 EV = win_prob × オッズ − 1。お買い得フラグ: 🎯 高期待値(EV≥20%)/
      ⭐ お買い得(≥10%)/ 💡 プラス期待値(>0、オッズ ≥ 2.0 倍)。閾値は引数で
      差し替え可能(サイドバースライダー / バックテスト感度分析)。
    - `kelly_fraction(win_prob, odds)`(Phase 4 用、Phase 2 では計算のみ)
    - `EV_SCHEMA_VERSION = "v1-ev-phase2"`

  * **統合(prediction_logic.py)**: `RacePrediction.horse_evs` 追加、
    `predict_race_v2` / `predict_race_dc` 末尾で公正オッズ版 EV をベイク
    (マーケットオッズなし)。UI でオッズ入力時に再計算。

  * **UI(app.py)**:
    - 確率ランキング表に「公正単勝/複勝オッズ」列(常時、ウィジェット不要)
    - 本命バナーに公正単勝オッズ併記
    - **「💰 期待値(EV)計算」ワークベンチ**(予想結果下部): レース選択 →
      `st.data_editor` で単勝オッズ入力 → EV + お買い得フラグを即時計算。
      入力値は `session_state["market_odds"]` に保持(ファイル変更で自動クリア)
    - **「💡 お買い得馬一覧」**: オッズ入力済みの全レース横断で EV>0 を集計
    - サイドバー「💰 EV 表示設定」: 高/中期待値の閾値スライダー
    - **設計判断(spec からの変更)**: spec 3a は「34 レース全部の表に編集可能
      オッズ列」だが、Streamlit は折りたたみ expander の中身も毎 rerun で実行
      する(v1.7.0 で独自 overlay を撤去した教訓)ため、34×data_editor は重い。
      よって公正オッズは常時表示・オッズ入力 + EV は 1 ワークベンチに集約した。
    - ロジック説明ページ(pages/01)に Phase 2 セクション(公正オッズ/EV 数式)

  * **regression(v1.12.0 完全一致)**(backtest_rating_v18、336 レース):
    ◎本命 84 / 1着率 14.29% / 複勝率 29.76% / 単勝参考回収率 695.4% =
    v1.12.0 と **±0.00 完全一致**(EV は判定/買い目/rating を読むだけの純粋追加)。

  * **⚠️ EV 戦略バックテストは検証不能(重要な発見)**:
    `scripts/backtest_ev_v13.py`(2026-04-01〜05-10、408 有効レース)で 6 戦略を
    比較したが、**historical の `odds` 列が実単勝オッズではない**ことが確定:
    - 全 odds 中央値 48.6 / 1着馬 odds 中央値 **49.9**(実単勝なら勝ち馬は ~3-5 倍)
    - レース毎 **Σ(1/odds) = 0.328**(健全な単勝本なら 1.1-1.3)
    - 結果、EV 平均 **+2.7**(あり得ない高さ)→ 全 EV 戦略の回収率 300%+ は
      odds 由来のアーティファクト。Baseline 677% も同じ別指標由来(全 backtest で
      「単勝参考回収率」と注記済みの既知事象)。
    - 戦略比較(参考、odds が別指標のため無意味): Baseline 677% に対し
      A:EV>0 311% / B:EV>0.10 312% / C:EV>0.20 316% / D:最高EV 519% /
      E:Kelly 424% と **全戦略 Baseline 未満**。
    - **判定**: EV 戦略が Baseline を上回るかは **このデータでは検証不能**
      (odds が別指標のため)。EV 機能の設計欠陥ではなく **データ品質の限界**。
    - **採用 EV 閾値**: バックテストで最適化できないため、保守的な既定値
      (🎯 20% / ⭐ 10%)を維持。お父様はサイドバーで調整可能。
    - **将来(Phase 2.1 候補)**: JRA-VAN DataLab 等から **実単勝オッズ** を
      historical に補完できれば EV 戦略の回収率改善を正しく検証可能。
    - ユーザー協議のうえ、Phase 2 は **ライブ決定支援ツール**(公正オッズ常時 +
      手入力オッズ EV)として main にマージ(regression ゼロ・純粋追加のため安全)。

  * **テスト**: 全 **285 pass**(既存 247 + 新規 38: ev_engine 24 + 統合 8 +
    バックテスト関数 6)。Streamlit 起動 smoke 無エラー / AST OK。

  * **将来の誤認防止メモ**:
    - **historical の `odds` 列は実単勝オッズではない**(別指標、Σ(1/odds)≈0.33、
      全頭 50〜60 台)。EV やオッズ系の backtest をする時は必ずこの事実を踏まえる。
      回収率系の数値は全て「参考」。
    - EV 機能のライブ運用(公正オッズ + 手入力オッズ)は有効。検証できないのは
      「履歴 odds を使った戦略の回収率」だけ。
    - 確率・rating は不変。EV は `utils/ev_engine.py` の派生で、
      rating_engine / probability_engine には書き込んでいない。

  * v1.4〜v1.12.0 の全機能(rating 骨格 / 性能 / G ルール / 脚質多段 / parquet 更新 /
    確率派生)完全維持、session_state 既存キー破壊なし。
- **v1.13.1 fix**(現行、2026-05): **DC 形式の開催回コード英数字対応**(お父様の
  5/30 当日出馬表が読み込めなかった問題の修正)。UI/検出のみ、ロジック完全不変。

  * **症状**: 5/30(東京・京都の 2 会場開催)の DC CSV をアップロードすると
    予想が動かない。実機再現で `load_race_card` が `data_format="header_csv"` に
    誤判定 → 1 行目をヘッダとして読み `racecourse` 列を喪失 → `KeyError`。

  * **真因(推測でなく実機確認)**: `utils/target_format.is_dc_format` が
    col[0] 全 10 桁を `isdigit()` で要求していた。しかし TARGET の DC 形式は
    **col[0] の開催回コード(4-6 桁目)が英数字になりうる**。
    - 失敗ファイル: col[0] 例 `05262B0101` → 開催回 `2B`/`3B`(英字 B 含む)
    - 既存動作ファイル: 開催回 `13`/`25`/`35`(数値)
    - 場コード(0-2桁)/ 年(2-4桁)/ R番(6-8桁)/ 馬番(8-10桁)は **数値**。
      英字は開催回 4-6 桁目だけ。
    - ⚠️ **「複数会場だから」ではない**: 既存 `dc_format_sample.csv` は
      **3 会場(新潟/東京/京都)** を含み従来から正常動作。会場数は無関係で、
      真因は「英数字の開催回コード」のみ。

  * **修正(`is_dc_format` のみ、parse は元から英数字対応済)**:
    - col[0] 全体の `isdigit()` 要件を撤廃
    - 代わりに: col[0] が 10 文字 / 先頭 2 桁が既知 JRA 場コード(01〜10)/
      年(2-4)・R番(6-8)・馬番(8-10)が数字 / col[1..9] が数値、で判定
    - `parse_dc_dataframe` は開催回コードを horse_id 用の opaque 文字列として
      扱うのみなので無改変で英数字 OK(`DC-05-2B-01-01` のように保持)

  * **JRA 場コード → 漢字名マッピング(完全版、`JRA_COURSE_BY_CODE`)**:
    01 札幌 / 02 函館 / 03 福島 / 04 新潟 / 05 東京 / 06 中山 / 07 中京 /
    08 京都 / 09 阪神 / 10 小倉。
    (race_id は `R<YYYYMMDD>-<場頭1文字><RR>`。中山/中京は既知の「中」衝突あり、
    今回の東京/京都は衝突なし。)

  * **実機検証(`data/test/dc_format_multi_venue_sample.csv`、5/30)**:
    - is_dc_format: False → **True** に修正
    - 読込: `data_format="dc"`、361 頭 / **24 レース(東京 12 + 京都 12)**
    - race_id 衝突なし(各 race_id 内の会場は一意)、◎本命 8 レース確定
    - 確率(v1.12.0)/ EV(v1.13.0)も会場をまたいで正常派生、実名マッチ 75.3%
      (DC 構造上限 77-78% 内)

  * **regression(完全一致)**:
    - 既存 DC `DC260509.CSV`: 実名マッチ **385/495 = 77.8%**(文書値と一致)、
      data_format=dc 維持、3 会場デコード維持
    - 検出: 既存 DC 3 ファイル → True、RA+SE/header(52列)→ False を維持
    - rating backtest(336 レース): is_dc_format は backtest 経路に無関係のため
      v1.13.0 と同一(◎84 / 1着14.29% / 複勝29.76% / 回収695.4%)
    - 全 **295 テスト pass**(既存 285 + 新規 10: 英数字開催回検出 / 複数会場
      読込・会場独立 / 場コード検証 / parse decode 等)

  * **お父様への影響**: **操作は今まで通り**(DC 形式 CSV をドラッグ&ドロップ)。
    これまで読めなかった「回次が進んだ開催日(英数字開催回コード)」の CSV が
    読めるようになった。

  * **将来の誤認防止メモ**: DC の col[0] は **開催回コード(4-6桁)が英数字に
    なりうる**。col[0] 全体を数値前提にしないこと。会場数は判定に無関係
    (元から複数会場対応済)。
